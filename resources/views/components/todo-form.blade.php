    ...
    <form method="POST" action="{{ url('create-todo') }}" class="max-w-lg mx-auto">
        <div class="flex flex-col gap-5">
            <div class="w-full py-2">
                ...
                <input name="name" type="text" placeholder="Task name" class="w-full px-5 rounded focus:ring-0">
                @error('name')
                    <small class="text-red-400">{{ $message }}</small>
                @enderror
            </div>
            <div class="w-full ">
                <textarea placeholder="Task description" name="description" class="w-full rounded resize-none focus:ring-0"></textarea>
                @error('description')
                    <small class="text-red-400">{{ $message }}</small>
                @enderror
            </div>
            <div class="w-full">
                ...
                <textarea placeholder=" long Task description" name="long_description" class="w-full rounded resize-none focus:ring-0"></textarea>
